...
type
	...
	public
		...
		{Encryption routines}
		// Encrypts the data in a 64-bit block using the ECB mode
		procedure EncryptECB(InData, OutData: Pointer);
		// Encrypts the data in a 64-bit block using the CBC chaining mode
		procedure EncryptCBC(InData, OutData: Pointer);
		// encrypts the data in a 64-bit block using the OFB chaining mode
		procedure EncryptOFB(InData, OutData: Pointer);
		// encrypts Len bytes of data using the CFB chaining mode
		procedure EncryptCFB(InData, OutData: pointer; Len: integer);
		// encrypts Len bytes of data using the OFB counter chaining mode
		procedure EncryptOFBC(InData, OutData: pointer; Len: integer);

		{Decryption routines}
		// Decrypts the data in a 64-bit block using the ECB mode
		procedure DecryptECB(InData, OutData: Pointer);
		// Decrypts the data in a 64-bit block using the CBC chaining mode
		procedure DecryptCBC(InData, OutData: Pointer);
		// Decrypts the data in a 64-bit block using the OFB chaining mode
		procedure DecryptOFB(InData, OutData: Pointer);
		// Decrypts Len bytes of data using the CFB chaining mode
		procedure DecryptCFB(InData, OutData: pointer; Len: integer);
		// Decrypts Len bytes of data using the OFB counter chaining mode
		procedure DecryptOFBC(InData, OutData: pointer; Len: integer);
...
{$IFDEF UnitTests}
type
	// Test methods for class TCast128
	TestTCast128 = class(TTestCase)
	strict private
		FCast128: TCast128;
	public
...
function TCast128.SelfTestB: Boolean;
type
...
var
	...
	i: Integer;
const
	Initial:   TBlock = ($01,$23,$45,$67,$12,$34,$56,$78,$23,$45,$67,$89,$34,$56,$78,$9a);
	ExpectedA: TBlock = ($EE,$A9,$D0,$A2,$49,$FD,$3B,$A6,$B3,$43,$6F,$B8,$9D,$6D,$CA,$92);
	ExpectedB: TBlock = ($B2,$C9,$5E,$B0,$0C,$31,$AD,$71,$80,$AC,$05,$B8,$E8,$3D,$69,$6E);
begin
(*
...
*)
	...
	begin
		Self.Init(@b[0], 16, nil);
		EncryptECB(aL, aL);
		EncryptECB(aR, aR);

		Self.Init(@a[0], 16, nil);
		EncryptECB(bL, bL);
		EncryptECB(bR, bR);
	end;

	Result := CompareMem(@a[0], @ExpectedA[0], 16) and
			CompareMem(@b[0], @ExpectedB[0], 16);

	if (not Result) then
		raise Exception.Create('CAST-128 full maintenance self-test failed');
end;
...
//*          IV: Pointer - to array[0..15] of byte - IV or nil         *
//**********************************************************************
procedure TCast128.Init(Key: Pointer; Len: Integer; IV: Pointer);
var
	x, t, z: array[0..3] of LongWord;
	i: Integer;
begin
...
{$Q-}
	{
		Encrypt the 8-byte block pointed to by InData.
	}

	if not FInitialized then
		raise Exception.Create('TCast128 object must be initialized before(.Init) before it can be used.');

	// Split 8-byte plaintext into 4-byte halves. [InData] ==> [l][r]
   l := PCardinal2(InData).Lo;
	r := PCardinal2(InData).Hi;
	...
	//Copy L and R halves to OutData;
	PCardinal2(OutData).Lo := r;
	PCardinal2(OutData).Hi := l;
end;
...
begin
	...
	r := PCardinal2(InData).Lo;
	l := PCardinal2(InData).Hi;

	// Swap L and R to big-endian
	l := (l shr 24) or ((l shr 8) and $FF00) or ((l shl 8) and $FF0000) or (l shl 24);
	r := (r shr 24) or ((r shr 8) and $FF00) or ((r shl 8) and $FF0000) or (r shl 24);
	...
	// Store l and r as two halves of OutData
	PCardinal2(OutData).Lo := l;
	PCardinal2(OutData).Hi := r;
end;

procedure TCast128.EncryptCBC(InData, OutData: Pointer);
begin
	if not FInitialized then
		raise Exception.Create('TCast128 object must be initialized before(.Init) before it can be used.');
...
procedure TCast128.DecryptCFB(InData, OutData: pointer; Len: integer);
var
	i: Integer;
	tempBlock: array[0..7] of byte;
	b: Byte;
...
procedure TCast128.EncryptOFB(InData, OutData: Pointer);
begin
	if not FInitialized then
		raise Exception.Create('TCast128 object must be initialized before(.Init) before it can be used.');
...
procedure TestTCast128.SetUp;
begin
  inherited;

  FCast128 := TCast128.Create;
end;

procedure TestTCast128.TearDown;
begin
	FreeAndNil(FCast128);
...
initialization
{$IFDEF UnitTests}
	RegisterTest('Avatar Library\Cast128 Tests', TestTCast128.Suite);
{$ENDIF}
